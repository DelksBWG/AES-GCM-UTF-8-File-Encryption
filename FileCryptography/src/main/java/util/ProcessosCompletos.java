...
public static void Criptacao(Scanner sc) { //pegando um arquivo e o criptografando em AES-GCM
...
public static void CriptacaoString(Scanner sc) { //pegando um string e o criptografando em AES-GCM
	try {
    ...
    GCMParameterSpec gcmParameterSpec = Conversao.GerandoGCM(vi.getIv().getIV());
    ChaveGCM gcm = new ChaveGCM(gcmParameterSpec);
    String textoCriptografado = PegandoArquivo.adquirindoString(chaveFinal, gcm.getGcmTag(), algoritmo, texto, new File("C:\\Users\\Delks\\User.txt"));
...
public static void Decriptacao(Scanner sc) { //pegando um arquivo e o descriptografando em AES-GCM
	String caminho = new String(AdquirindoArquivo.ListandoEAdquirindo(sc));
	try {
	Arquivo arquivoRecebido = new Arquivo(new File(caminho));
    System.out.print("Escreva a chave: ");
	SecretKey chave = Conversao.StringParaChave(sc.next());
    ChaveCodificar chaveFinal = new ChaveCodificar(chave);
    String chaveString = Conversao.ChaveParaString(chaveFinal.getChave());
    System.out.println(chaveString);
    System.out.print("Escreva o IV: ");
    IvParameterSpec ivParameterSpec = new IvParameterSpec(Conversao.StringParaByte(sc.next()));
    VetorInicializador vi = new VetorInicializador(ivParameterSpec);
    String ivString = Conversao.ByteParaString(vi.getIv().getIV());
    System.out.println(ivString);
    String algoritmo = "AES/GCM/NoPadding";
    ...
    PegandoArquivo.retornando(chaveFinal, gcm.getGcmTag(), algoritmo, arquivoRecebido, resultado);
		...
		System.out.println(caminho + " Não funcionou por " + e);
		sc.close();
		}
}
public static void DecriptacaoString(Scanner sc) { //pegando um string e o descriptografando em AES-GCM
	try {
		System.out.print("Me conte seu segredo ");
	String texto = sc.nextLine();
    ...
    GCMParameterSpec gcmParameterSpec = Conversao.GerandoGCM(vi.getIv().getIV());
    ChaveGCM gcm = new ChaveGCM(gcmParameterSpec);
    String textoDecriptografado = PegandoArquivo.retornandoString(chaveFinal, gcm.getGcmTag(), algoritmo, texto, new File("C:\\Users\\Delks\\User.txt"));
