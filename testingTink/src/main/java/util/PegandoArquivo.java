...
public static void adquirindo(ChaveCodificar chave, GCMParameterSpec gcm, String algoritmo, Arquivo arquivo, //classe para alterar o File na classe arquivo utilizando a Cipher cifra
	...
	String arquivoString = Conversao.FileParaString(arquivo.getAceito(), "UTF-8");
	adquirindoString(chave, gcm, algoritmo, arquivoString, destino.getArquivo());
    arquivo.getAceito().close(); //fechando as Streams do Input e Output 
    destino.getAceito().close();
}
public static String adquirindoString(ChaveCodificar chave, GCMParameterSpec gcm, String algoritmo, String arquivo, File arq //classe para alterar o File na classe arquivo utilizando a Cipher cifra
...
public static void retornando(ChaveCodificar chave, GCMParameterSpec gcm, String algoritmo, Arquivo arquivo, //classe para alterar o File na classe arquivo utilizando a Cipher cifra
	...
	String arquivoString = Conversao.FileParaString(arquivo.getAceito(), "UTF-8");
	retornandoString(chave, gcm, algoritmo, arquivoString, arquivo.getArquivo());
    arquivo.getAceito().close(); //fechando as Streams do Input e Output 
    destino.getAceito().close();
}
public static String retornandoString(ChaveCodificar chave, GCMParameterSpec gcm, String algoritmo, String arquivo, File arq //classe para alterar o File na classe arquivo utilizando a Cipher cifra
		 ) throws IOException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		    Cipher cifra = Cipher.getInstance(algoritmo); //inicializar a Cipher cifra na inst�ncia do String algoritmo
		    ...
		    String textoCifradoString = Conversao.ByteParaString(textoCifrado);
		    testeOutputDecriptacao(textoCifradoString, arq);
		    return textoCifradoString;
		}
...
public static void testeOutputEncriptacao(String teste, File arq) throws IOException { //para escrever em um arquivo sem alterações do texto do que está escrito em uma string
  ...
  public static void testeOutputDecriptacao(String teste, File arq) throws IOException { //para escrever em um arquivo com alterações do texto do que está escrito em uma string usando byte array
